...
public class CraftsiScreen extends Screen {
    ...
    static {
        ...
        RU.put("камень", "stone");
        RU.put("земля", "dirt");
        RU.put("железо", "iron_ingot");
        RU.put("золото", "gold_ingot");
        RU.put("алмаз", "diamond");
        RU.put("уголь", "coal");
        RU.put("факел", "torch");
        RU.put("верстак", "crafting_table");
        RU.put("печь", "furnace");
        RU.put("сундук", "chest");
        RU.put("стекло", "glass");
        RU.put("шерсть", "white_wool");
        RU.put("кирпич", "brick");
        RU.put("книга", "book");
        RU.put("лук", "bow");
        RU.put("стрела", "arrow");
        ...
        RU.put("кожа", "leather");
        RU.put("кость", "bone");
        RU.put("порох", "gunpowder");
        RU.put("дубовое бревно", "oak_log");
    ...
    // Убираем блюр только через blursBackground — renderBackground НЕ переопределяем
    @Override
    ...
    private void renderLeftPanel(DrawContext ctx, int mx, int my) {
        ...
        for (int i = 0; i < TOOLS.length; i++) {
            ...
            boolean act = tool == i;
            ctx.fill(4, ty, LW - 4, ty + 20, act ? BLUD : (hov ? NODEHOV : PDARK));
    ...
    private void renderCanvas(DrawContext ctx, int mx, int my) {
        ...
        for (RecipeNode nd : NodeGraph.getInstance().getNodes())
            drawNode(ctx, nd, mx, my);
    ...
    private void drawNode(DrawContext ctx, RecipeNode n, int mx, int my) {
        ...
        ctx.drawBorder(x, y, w, h, n.autoCraftOn ? (n.isActive ? GRN : RED) : BDRL);
        ...
        if (zoom >= 0.55f) {
            ...
            if (n.craftCount > 0)
                ctx.drawTextWithShadow(textRenderer,
                    Text.literal("ok " + n.craftCount), x + 4, y + h - 24, GRN);
            ...
            if (n.autoCraftOn && !n.isActive) {
                ...
                if (mc.player != null) {
                    ...
                    if (!miss.isEmpty())
                        ctx.drawTextWithShadow(textRenderer,
                            Text.literal("x " + fit(miss, w - 8)), x + 4, y + h - 14, RED);
                }
            }
    ...
    private void renderEditPanel(DrawContext ctx, int mx, int my) {
        ...
        ctx.drawCenteredTextWithShadow(textRenderer, Text.literal("->"), px + 152, py + 84, YEL);
        ...
        if (!editNode.result.isEmpty()) {
            ...
            ctx.drawTextWithShadow(textRenderer,
                Text.literal("x" + editNode.result.getCount()), rx2 + 28, ry2 + 30, YEL);
        } else {
            ctx.drawCenteredTextWithShadow(textRenderer,
        ...
        ctx.drawCenteredTextWithShadow(textRenderer,
            ...
            px + 105, py + 185, WHITE);
        ctx.drawTextWithShadow(textRenderer,
            Text.literal("Создано: " + editNode.craftCount), px + 8, py + 202, GRAY);
        ...
        ctx.drawTextWithShadow(textRenderer,
            Text.literal(inputBuf.isEmpty() ? "Введи рус/англ..." : inputBuf + "|"),
    ...
    private void renderItemPicker(DrawContext ctx, int mx, int my) {
        ...
        ctx.drawTextWithShadow(textRenderer,
            Text.literal("Поиск: " + (searchQ.isEmpty() ? "рус/англ..." : searchQ + "|")),
        ...
        for (int i = 0; i < filtItems.size() - startIdx; i++) {
            ...
            boolean hov = mx >= ix && mx <= ix + PSLOT - 2
                && my >= iy && my <= iy + PSLOT - 2;
            if (hov) hovItem = item;
            ctx.fill(ix, iy, ix + PSLOT - 2, iy + PSLOT - 2, hov ? 0xFF0A1E3A : PDARK);
        ...
        if (hovItem != null)
            ctx.drawTooltip(textRenderer,
                Text.literal(hovItem.toString().replace("minecraft:", "")), mx, my);
    ...
    public boolean mouseScrolled(double mx, double my, double h, double v) {
        if (panel == Panel.ITEM_PICKER) {
            ...
            if (mx >= px && mx <= px + pw && my >= py && my <= py + ph) {
                int rows = (ph - 80) / PSLOT;
                int maxS = (int) Math.ceil(filtItems.size() / (double) PCOLS) - rows;
