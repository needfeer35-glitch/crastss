...
public class CraftsiScreen extends Screen {
    ...
    // ══ КРИТИЧНО: убираем блюр и затемнение ══
    @Override
    public boolean blursBackground() { return false; }

    @Override
    public void renderBackground(DrawContext ctx, int mx, int my, float delta) {
        // Полностью пусто — не вызываем super, не рисуем блюр
    }
    ...
    private void renderEditPanel(DrawContext ctx, int mx, int my) {
        ...
        for (int i=0;i<9;i++) {
            int col=i%3,row=i/3,sx=px+8+col*46,sy=py+38+row*46;
            boolean sel=editingSlot==i;
            ctx.fill(sx,sy,sx+42,sy+42, sel?0xFF0A2010:PANELDARK);
            ctx.drawBorder(sx,sy,42,42, sel?GREEN:BORDER);
            ItemStack s=editingNode.inputs[i];
            if (s!=null&&!s.isEmpty()) {
                ctx.drawItem(s,sx+4,sy+4);
                ctx.drawTextWithShadow(textRenderer,Text.literal("×"+s.getCount()),sx+28,sy+30,YELLOW);
            } else {
                ctx.drawCenteredTextWithShadow(textRenderer,Text.literal(String.valueOf(i+1)),sx+21,sy+17,GRAY);
            }
        }
        ...
        ctx.drawTextWithShadow(textRenderer,
            Text.literal("Создано: "+editingNode.craftCount),px+8,py+202,GRAY);
        ...
        ctx.drawTextWithShadow(textRenderer,
            Text.literal("напр: палка / stick / oak_log"),rx,py+58,GRAY);
    ...
    private void renderItemPicker(DrawContext ctx, int mx, int my) {
        ...
        if (totalRows>visRows) {
            ...
            int thumbH=Math.max(10,(int)((float)visRows/totalRows*trackH));
            int thumbY=py+44+(int)((float)pickerScroll/Math.max(1,totalRows-visRows)*(trackH-thumbH));
    ...
    public boolean charTyped(char c,int mods) {
        if (activePanel==Panel.ITEM_PICKER) {
            searchQuery+=c; rebuildFilter(); return true;
        }
        if (activePanel==Panel.EDIT_NODE&&editingSlot>=0) {
            inputBuffer+=c; return true;
        }
        return false;
    }
